...
// ImageMessage component for handling authenticated image display
function ImageMessage({ message, mine }) {
  ...
  useEffect(() => {
    const loadImage = async () => {
      ...
      try {
        ...
        console.log('Loading authenticated image for ID:', mediaId);
        ...
        console.log('Successfully loaded authenticated image:', authenticatedBlobUrl);
      } catch (error) {
...
export default function ChatPage({ user, onLogout }){
  ...
  const uploadImage = async (file) => {
    ...
    try {
      ...
      // Use relative URL so it works with authentication properly
                    ...
                    >
                      ...
                      <div className={`max-w-xs md:max-w-sm lg:max-w-md ${isMe ? 'order-1' : ''}`}>
                        ...
                        >
                          ...
                          {m.type === 'image' && (
                            <ImageMessage message={m} mine={isMe} />
                          )}
